package study.spring.zigme.helper;
...
public class WebHelper {
	...
	public String getUploadUrl(String filePath) {
		...
		// window의 경우 경로 구문을 역슬래시로 하는데, 이는 웹에 노출 할 수 있는 형태가 아니므로 역슬래시를 슬래시로 변환하여 반환한다.
		return path.replace("\\", "/");
	}

	/**
	 * 컨트롤러로부터 업로드 된 파일의 정보를 전달받아 지정된 위치에 저장한다. 이 때, 파일 덮어쓰기를 방지하기 위해 파일의 이름을 변경하고
	 * 이름 중복 검사를 수행한다.
	 * 
